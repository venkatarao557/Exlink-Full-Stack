...
INSERT INTO #E06Source (CurrencyUnit, CurrencyDescription) VALUES
...
INSERT INTO [exlink].[Currency] (CurrencyUnit, CurrencyDescription)
SELECT s.CurrencyUnit, s.CurrencyDescription
FROM #E06Source s
WHERE NOT EXISTS (
    SELECT 1 FROM [exlink].[Currency] t 
...
IF @CurrInsertCount > 0
    PRINT CAST(@CurrInsertCount AS NVARCHAR(10)) + ' records inserted successfully into [exlink].[Currency].';
ELSE
    PRINT 'No new records were inserted into [exlink].[Currency] (all records already exist).';
